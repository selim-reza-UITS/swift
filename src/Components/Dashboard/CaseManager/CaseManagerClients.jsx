...
const CaseManagerClients = () => {
  ...
  const filteredClients = clients.filter(client => {
    ...
    if (activeView === 'active') return matchesSearch && client.status === 'active';
    if (activeView === 'paused') return matchesSearch && client.status === 'paused';
    if (activeView === 'recovery') return matchesSearch && client.status === 'recovery';
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'active': return 'bg-[#A855F7]';
      case 'paused': return 'bg-[#F59E0B] text-[#161E2F]';
      case 'recovery': return 'bg-[#EF4444] text-[#161E2F]';
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      ...
      case 'medium': return 'bg-[#FFEDD5] text-[#9A3412]';
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex items-center space-x-4">
                <div className="relative">
                  ...
                  />
                  <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-gray-800 ${getStatusColor(client.status)}`}></div>
                </div>
              ...
              <div className="flex items-center space-x-3">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(client.status)} text-white`}>
          ...
          {currentClients.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-400 text-lg">No clients found matching your criteria.</p>
            </div>
          )}
        </div>
      </div>
